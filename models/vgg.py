...
class DropReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        mean_val, _ = torch.max( x.view(x.size(0), x.size(1), -1) ,dim=-1) # [B, C]
        mean_val = mean_val[:, :, None, None].expand_as(x)  # [B, C, H/8, W/8]
            
        x = torch.min(x, mean_val*self.thresh)
...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=20, 
        ...
        self.activation = DropReLU(thresh)
...
if __name__ == '__main__':
    ...
    model = vgg16(pretrained=True, thresh=0.8)
