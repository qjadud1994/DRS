...
def train(current_epoch):
    ...
    for idx, dat in enumerate(train_loader):
        ...
        if global_counter % args.show_interval == 0:
            ...
            result_cam = visualize(img[0], cam[0].cpu().detach().numpy(), label[0])
            writer.add_images('train CAM', result_cam, global_counter)
            
            result_cam = visualize(img[0], (att_map[0].cpu().detach().numpy(), label[0])
            writer.add_images('train GT', result_cam, global_counter)
